...
(async () => {
  ...
  for (let i = 0; i < SELECTED.length; i++) {
    const inP = path.join(SRC, `DSC_${SELECTED[i]}.JPG`);
    if (!fs.existsSync(inP)) { missing.push(SELECTED[i]); continue; }
    n++;
  ...
  if (missing.length) console.warn("MISSING from selected-photos:", missing.join(", "));
